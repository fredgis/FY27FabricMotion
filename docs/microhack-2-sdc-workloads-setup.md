...
| **Changed the UI / Copilot upgraded it, but Fabric still shows the old version** | Browser / Fabric workload-iframe **cache** (Ctrl+Shift+R doesn't always bust it); or new files/views not bundled | Restart the **Dev Server** (for new files/views), then **F12 → Network → Disable cache** + reload, or "Empty Cache and Hard Reload", or close & reopen the item / Incognito. (No Dev Gateway restart for UI changes.) |
