...
| **Item goes blank right after adding `onelake.ts` (Step 8)** | A **compile error** in `onelake.ts` breaks the bundle — wrong auth API or a non-ASCII char from copy-paste | Use `client.auth.acquireFrontendAccessToken({ scopes: ['https://storage.azure.com/user_impersonation'] })` (not `acquireAccessToken({ additionalScopesToConsent })`); re-type any Unicode hyphen `‐` in `@ms-fabric/workload-client` and stray `⌋`/`↪`/line-breaks in the URL back-tick as ASCII; confirm `npx tsc --noEmit` is green |
| **Scorecard shows "Failed: …" (not blank) after wiring OneLake** | Build is fine — **runtime** OneLake fetch issue | Check token consent, that `Files/sites.csv` exists, a 403, or CORS — debug the fetch, not the build. Keep the seed line as a fallback to keep moving |
